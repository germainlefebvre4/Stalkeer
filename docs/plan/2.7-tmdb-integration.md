...
Implement TMDB (The Movie Database) API integration to enrich parsed M3U playlist items with authoritative metadata including titles, years, posters, overviews, genres, TVDB IDs, and additional information for movies and TV shows.
...
- [x] TMDB API client successfully searches for movies and TV shows
- [x] API client implements retry logic and circuit breaker pattern
- [x] Search methods return movie and TV show results with metadata
- [x] GetMovieDetails() and GetTVShowDetails() retrieve full information
- [x] GetMovieExternalIDs() and GetTVShowExternalIDs() fetch TVDB IDs
- [x] Enrichment integrated into processor for automatic enrichment during processing
- [x] Matching algorithm extracts title and year from playlist items
- [x] Movies table populated with TMDB ID, title, year, genres, duration, TVDB ID
- [x] TVShows table populated with TMDB ID, title, year, genres, season, episode, TVDB ID
- [x] Deduplication prevents duplicate Movie/TVShow records
- [x] Configuration supports API key and language from config file
- [x] Error handling covers TMDB API error scenarios
- [x] Unit tests cover TMDB client methods
- [x] Integration with process command for automatic enrichment
- [ ] CLI enrich command for bulk enrichment of existing items
- [ ] API endpoints for triggering enrichment jobs
...
- ✅ Full TMDB client implementation in `/internal/external/tmdb/tmdb.go`
- ✅ Search methods: `SearchMovie()`, `SearchTVShow()`
- ✅ Details methods: `GetMovieDetails()`, `GetTVShowDetails()`
- ✅ External IDs methods: `GetMovieExternalIDs()`, `GetTVShowExternalIDs()` for TVDB integration
- ✅ Circuit breaker pattern for API resilience
- ✅ Retry logic with exponential backoff
- ✅ Configurable language support for international titles
- ✅ Helper functions: `ExtractYear()`, `FormatGenres()`
- ✅ Integration with processor for automatic enrichment
- ✅ Title extraction and cleaning for TV shows
- ✅ Year parsing from movie titles
- ✅ Movies and TVShows models with TMDB and TVDB IDs
- ✅ ProcessedLine associations via MovieID and TVShowID
- ✅ Configuration in `config.yml` with enabled flag, API key, and language
- ✅ Statistics tracking (TMDBMatched, TMDBNotFound, TMDBErrors)

**Implementation Details:**
- TMDB client uses HTTP client with 30-second timeout
- Circuit breaker prevents cascading failures
- Language configuration supports formats like "fr-FR,fr;q=0.9,en-US;q=0.5,en;q=0.5"
- Enrichment happens during `process` command execution
- TMDB can be disabled via config or `--skip-tmdb` flag
- Both TMDB ID and TVDB ID are stored for cross-platform compatibility
- Season/episode information preserved for TV shows
- Genre data stored as JSON array

**Remaining Work:**
- ⏳ Standalone `enrich` CLI command for bulk enrichment
- ⏳ REST API endpoints for enrichment status and control
- ⏳ Caching layer for TMDB responses
- ⏳ Rate limiting implementation
- ⏳ Progress reporting for large enrichment jobs
