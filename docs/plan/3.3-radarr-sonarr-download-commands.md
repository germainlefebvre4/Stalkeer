...
Build CLI commands to fetch missing content from Radarr and Sonarr, match against the local database (movies/tvshows tables) using TMDB/TVDB metadata, and trigger downloads of matched stream URLs. This provides an automated workflow to download missing media directly from the M3U playlist sources.

### Success Criteria
- ✅ Missing movies/TV shows identified from Radarr/Sonarr
- ✅ Accurate matching against local database using TMDB and TVDB IDs
...
### Must Have
- [x] `stalkeer radarr` command fetches missing movies and downloads matches
- [x] `stalkeer sonarr` command fetches missing episodes and downloads matches
- [x] TMDB/TVDB-based matching for accurate identification
- [x] Dry-run mode accurately previews actions
- [x] Downloads tracked with state in database
- [x] Progress reporting during downloads
- [x] Configurable via `config.yml`
- [x] Error handling covers network, disk, and API failures
- [x] File organization follows Radarr/Sonarr conventions
- [x] Parallel downloads with configurable concurrency
- [x] Statistics reporting (matched, downloaded, failed, skipped)
- [x] Disk space validation before downloads
- [ ] Unit tests for matcher and downloader
- [ ] Integration tests for full workflows

### Nice to Have
- [x] Resume interrupted downloads
- [x] Post-download verification (file size)
...
- ✅ Radarr command in `/cmd/main.go` (lines 365-564)
  - Fetches missing movies from Radarr API
  - Matches using TMDB ID and TVDB ID
  - Fallback to fuzzy title matching
  - Downloads to: `{movies_path}/{Movie Title (Year)}/{Movie Title (Year)}.{ext}`
  - Flags: `--dry-run`, `--limit`, `--parallel`, `--force`, `--verbose`
  - Statistics: matched, downloaded, failed, skipped, not found

- ✅ Sonarr command in `/cmd/main.go` (lines 566-863)
  - Fetches missing episodes from Sonarr API
  - Matches using TMDB ID and TVDB ID with season/episode
  - Fallback to fuzzy title matching
  - Downloads to: `{tvshows_path}/{Series}/Season ##/{Series - S##E## - Episode}.{ext}`
  - Flags: `--dry-run`, `--limit`, `--parallel`, `--force`, `--verbose`, `--series-id`
  - Statistics: matched, downloaded, failed, skipped, not found

- ✅ Matcher enhancements:
  - TMDB ID matching for movies
  - TVDB ID matching for TV shows
...
- ✅ Downloader features:
  ...
  - Parallel downloads via ParallelDownloader
...
- ✅ File organization:
  ...
  - TV Shows: `{Series}/Season ##/{Series - S##E## - Episode}.{ext}`
  - Automatic directory creation
  - Proper file permissions

**Remaining Work:**
- ⏳ Comprehensive unit tests for commands
...
- [x] Created detailed download organization implementation plan (see `/github/plans/implementation/download-organization-plan.md`)
...
### Remaining Work
- [ ] **Improve download organization** (HIGH PRIORITY)
  - [ ] Download to system temp directory first
  - [ ] Detect file extensions from URL/Content-Type
  - [ ] Move files to configured directories after download
  - [ ] Handle cross-filesystem moves properly
...
### Known Issues
- **Files currently download directly to destination directory with `.tmp` suffix** (should use separate temp location)
- **File extensions hardcoded in CLI** (should be detected from URL/Content-Type)
- **No cleanup of orphaned temp files on startup**
- Parallel downloads not yet implemented (flag accepted but not used)
- TVDB ID used as proxy for TMDB ID in Sonarr - may need TVDB→TMDB mapping
