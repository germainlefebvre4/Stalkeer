...
### Problem Statement
Previously, Stalkeer used a single logging configuration for both application and database logging. The solution implements separate log level configuration for application and database components while maintaining backward compatibility.
...
- [x] New configuration format works correctly
- [x] Legacy configuration format still works with deprecation warning
- [x] Application logs use app log level
- [x] Database logs use database log level  
...
- ✅ Enhanced configuration structure in `/internal/config/config.go`:
  ```yaml
  logging:
    format: json  # or "text"
    app:
...
- ✅ Backward compatibility with legacy format:
  ```yaml
  logging:
    level: info  # Still works, with deprecation warning
    format: json
  ```

- ✅ Logger package enhancements in `/internal/logger/logger.go`:
  - `InitializeLoggersWithFormat()` for app and database loggers
  - `AppLogger()` returns application logger instance
  - `DatabaseLogger()` for GORM integration
  - Separate log levels for each component

- ✅ Config helper methods:
  - `GetAppLogLevel()` - returns app log level with fallback
  - `GetDatabaseLogLevel()` - returns database log level with fallback
  - `IsUsingLegacyLogging()` - checks if using old config format

- ✅ GORM adapter in `/internal/logger/gorm_adapter.go`:
  - Maps application log levels to GORM log levels
  - Uses database logger instance
  - Proper integration with GORM logging interface

- ✅ Server command integration:
  - Initializes both loggers on startup
  - Shows deprecation warning for legacy config
  - Displays active log levels in startup message
...
- ⏳ Complete manual testing across all commands
